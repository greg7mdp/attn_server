...
InitialSync::InitialSync(
    std::weak_ptr<AttnServer> federator,
    bool isMainchain,
    beast::Journal j)
...
InitialSync::done()
{
    ...
    {
        f->initialSyncDone(
            isMainchain_ ? ChainType::mainChain
                         : ChainType::sideChain);
    }
}
...
InitialSync::replay(std::lock_guard<std::mutex> const& l)
{
    ...
    if (auto f = federator_.lock())
    {
        auto const key = isMainchain_ ? UnlockMainLoopKey::mainChain
                                      : UnlockMainLoopKey::sideChain;
...
InitialSync::onEvent(event::TicketCreateResult&& e)
{
    ...
    if (needsOtherChainLastXChainTxn_)
    {
        if (auto f = federator_.lock())
        {
            ...
            ChainType const chainType = srcChainType(e.dir_);
...
InitialSync::onEvent(event::DisableMasterKeyResult&& e)
{
    ...
    if (needsOtherChainLastXChainTxn_)
    {
        if (auto f = federator_.lock())
        {
            ...
            ChainType const chainType = getChainType(!isMainchain_);
...
Json::Value getInfo(FederatorEvent const& event)
{
    return std::visit(
        [](auto const& e) {
            using eventType = decltype(e);
            Json::Value ret{Json::objectValue};
            if constexpr (std::is_same_v<eventType, event::XChainTransferDetected>)
            {
            ...
            else if constexpr (std::is_same_v<eventType, event::XChainTransferResult>)
            {
...
Json::Value InitialSync::getInfo() const
{
    Json::Value ret{Json::objectValue};
    {
        std::lock_guard l{m_};
